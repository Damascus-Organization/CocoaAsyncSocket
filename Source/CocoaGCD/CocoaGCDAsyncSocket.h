...
@class CocoaGCDAsyncSocketPreBuffer;
@protocol CocoaGCDAsyncSocketDelegate;

NS_ASSUME_NONNULL_BEGIN

extern NSString *const CocoaGCDAsyncSocketException;
extern NSString *const CocoaGCDAsyncSocketErrorDomain;

extern NSString *const CocoaGCDAsyncSocketQueueName;
extern NSString *const CocoaGCDAsyncSocketThreadName;

extern NSString *const CocoaGCDAsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const CocoaGCDAsyncSocketUseCFStreamForTLS;
#endif
#define CocoaGCDAsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define CocoaGCDAsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define CocoaGCDAsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
extern NSString *const CocoaGCDAsyncSocketSSLPeerID;
extern NSString *const CocoaGCDAsyncSocketSSLProtocolVersionMin;
extern NSString *const CocoaGCDAsyncSocketSSLProtocolVersionMax;
extern NSString *const CocoaGCDAsyncSocketSSLSessionOptionFalseStart;
extern NSString *const CocoaGCDAsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const CocoaGCDAsyncSocketSSLCipherSuites;
extern NSString *const CocoaGCDAsyncSocketSSLALPN;
#if !TARGET_OS_IPHONE
extern NSString *const CocoaGCDAsyncSocketSSLDiffieHellmanParameters;
#endif

#define CocoaGCDAsyncSocketLoggingContext 65535


typedef NS_ERROR_ENUM(CocoaGCDAsyncSocketErrorDomain, CocoaGCDAsyncSocketError) {
	CocoaGCDAsyncSocketNoError = 0,           // Never used
	CocoaGCDAsyncSocketBadConfigError,        // Invalid configuration
	CocoaGCDAsyncSocketBadParamError,         // Invalid parameter was passed
	CocoaGCDAsyncSocketConnectTimeoutError,   // A connect operation timed out
	CocoaGCDAsyncSocketReadTimeoutError,      // A read operation timed out
	CocoaGCDAsyncSocketWriteTimeoutError,     // A write operation timed out
	CocoaGCDAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	CocoaGCDAsyncSocketClosedError,           // The remote peer closed the connection
	CocoaGCDAsyncSocketOtherError,            // Description provided in userInfo
};
...
@interface CocoaGCDAsyncSocket : NSObject

/**
 * CocoaGCDAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, CocoaGCDAsyncSocket will automatically create it's own socket queue.
...
- (instancetype)initWithDelegate:(nullable id<CocoaGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id<CocoaGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq NS_DESIGNATED_INITIALIZER;

/**
 * Create CocoaGCDAsyncSocket from already connect BSD socket file descriptor
**/
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<CocoaGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError **)error;

#pragma mark Configuration

@property (atomic, weak, readwrite, nullable) id<CocoaGCDAsyncSocketDelegate> delegate;
...
- (void)getDelegate:(id<CocoaGCDAsyncSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id<CocoaGCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
...
**/
- (void)synchronouslySetDelegate:(nullable id<CocoaGCDAsyncSocketDelegate>)delegate;
- (void)synchronouslySetDelegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id<CocoaGCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;

/**
 * By default, both IPv4 and IPv6 are enabled.
 * 
 * For accepting incoming connections, this means CocoaGCDAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means CocoaGCDAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, CocoaGCDAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, CocoaGCDAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of CocoaGCDAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a CocoaGCDAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a CocoaGCDAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that CocoaGCDAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - CocoaGCDAsyncSocketManuallyEvaluateTrust
 ...
 *     So CocoaGCDAsyncSocket will invoke the delegate method socket:shouldTrustPeer: passing the SecTrustRef.
 *
 ...
 *
 * - CocoaGCDAsyncSocketUseCFStreamForTLS (iOS only)
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     By default CocoaGCDAsyncSocket will use the SecureTransport layer to perform encryption.
 ...
 *     
 *     However, if you absolutely must, you can instruct CocoaGCDAsyncSocket to use the old-fashioned encryption
 *     technique by going through the CFStream instead. So instead of using SecureTransport, CocoaGCDAsyncSocket
 ...
 *     
 *     Thus all the other keys in the given dictionary will be ignored by CocoaGCDAsyncSocket,
 ...
 *
 * - CocoaGCDAsyncSocketSSLProtocolVersionMin
 * - CocoaGCDAsyncSocketSSLProtocolVersionMax
 ...
 * 
 * - CocoaGCDAsyncSocketSSLSessionOptionFalseStart
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     See Apple's documentation for kSSLSessionOptionFalseStart.
 * 
 * - CocoaGCDAsyncSocketSSLSessionOptionSendOneByteRecord
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     See Apple's documentation for kSSLSessionOptionSendOneByteRecord.
 * 
 * - CocoaGCDAsyncSocketSSLCipherSuites
 ...
 *
 * - CocoaGCDAsyncSocketSSLDiffieHellmanParameters (Mac OS X only)
 *     The value must be of type NSData.
 ...
 *     You MUST use manual trust evaluation instead (see CocoaGCDAsyncSocketManuallyEvaluateTrust).
 ...
 *     You MUST use CocoaGCDAsyncSocketSSLProtocolVersionMin & CocoaGCDAsyncSocketSSLProtocolVersionMin instead.
...
/**
 * CocoaGCDAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(CocoaGCDAsyncSocket *)sock
 * {
 ...
 *     return socketQueue;
 * }
 * - (void)socket:(CocoaGCDAsyncSocket *)sock didAcceptNewSocket:(CocoaGCDAsyncSocket *)newSocket
 * {
 ...
 * Example usage:
 * 
 * - (void)socket:(CocoaGCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port
 * {
...
@protocol CocoaGCDAsyncSocketDelegate <NSObject>
@optional
...
**/
- (nullable dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(CocoaGCDAsyncSocket *)sock;
 ...
 * The host parameter will be an IP address, not a DNS name.
 **/
- (void)socket:(CocoaGCDAsyncSocket *)sock didConnectToUrl:(NSURL *)url;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)socket:(CocoaGCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)socket:(CocoaGCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(CocoaGCDAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used for things such as updating progress bars.
**/
- (void)socket:(CocoaGCDAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(CocoaGCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(CocoaGCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(CocoaGCDAsyncSocket *)sock;
 ...
 * 
 * Note: If the CocoaGCDAsyncSocket instance is deallocated while it is still connected,
...
**/
- (void)socketDidDisconnect:(CocoaGCDAsyncSocket *)sock withError:(nullable NSError *)err;
...
**/
- (void)socketDidSecure:(CocoaGCDAsyncSocket *)sock;

/**
 * Allows a socket delegate to hook into the TLS handshake and manually validate the peer it's connecting to.
 *
 * This is only called if startTLS is invoked with options that include:
 * - CocoaGCDAsyncSocketManuallyEvaluateTrust == YES
 *
...
**/
- (void)socket:(CocoaGCDAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
